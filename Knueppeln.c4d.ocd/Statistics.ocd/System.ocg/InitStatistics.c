...
static const STATS_Type_None = 0,
             STATS_Type_RoundType = 1 << 1,
             STATS_Type_ChampSelection = 1 << 2,
             STATS_Type_ChampKillsDeaths = 1 << 3,
             STATS_Type_ChampSpellUsage = 1 << 4,
             STATS_Type_All = ~0;

global func InitStatistics(int type)
{
	if (type & STATS_Type_RoundType)
		CreateObject(Statistics_RoundType);
